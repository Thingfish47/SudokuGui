...
void Sudoku::buttonClicked(Button* buttonThatWasClicked)
{
    ...
    else if (buttonThatWasClicked == bnEnter.get())
    {
        ...
        bnClear->setVisible(bnNotes->getToggleState() ? false : true);
    else if (buttonThatWasClicked == bnTest.get())
        handleTest();
...
void Sudoku::handleTest()
{
    String fn;
    for (int i = 0; i < N; i ++)
        fn << String (bnCells[i]->getCurrentValue());
    fn += ".txt";
    createLogFile(fn);
}
...
void Sudoku::createLogFile(String filename)
{
    File f = juce::File::getSpecialLocation(juce::File::SpecialLocationType::userDocumentsDirectory);
    String fname = f.getFullPathName();
    fname += "\\" + filename;
    Logfile.reset(new FileOutputStream(fname));
    Logfile->setPosition(0);
    Logfile->truncate();
}

void Sudoku::log(String text)
{
    if (Logfile.get() != nullptr)
    {
        text += "\n";
        Logfile->writeText(text, false, false, nullptr);
        Logfile->flush();
    }
}
...
void Sudoku::solveTheSudoku()
{
	...
	for (int col = 0; col < N; col++)
		if (onlyInCol(col) == true)
			hit = true;
	for (int square = 0; square < N; square++)
		if (onlyInSquare(square) == true)
			hit = true;
    //
	setMatchedPairs();
	for (int row = 0; row < N; row++)
		if (check_Pairs_In_Row(row))
			hit = true;
    //
    setMatchedPairs();
    for (int col = 0; col < N; col++)
    if (check_Pairs_In_Col(col))
        hit = true;
    //
    setMatchedPairs();
    for (int sqr = 0; sqr < N; sqr++)
        if (check_Pairs_In_Square(sqr))
            hit = true;
    //
    for (int row = 0; row < N; row++)
        if (check_Hidden_Matched_Pairs_In_Row(row))
            hit = true;
    //
    for (int col = 0; col < N; col++)
        if (check_Hidden_Matched_Pairs_In_Col(col))
            hit = true;
    //
    for (int col = 0; col < N; col++)
        if (check_Hidden_Pairs_In_Square(col))
            hit = true;
    //
    for (int sqr = 0; sqr < N; sqr++)
        if (check_Hidden_Matched_Pairs_In_Square(sqr))
            hit = true;
    if (hit)
    {
        if (solved())
...
void Sudoku::handleFindPairs()
{
    ...
    if (hit)
    {
        ...
        {
            DBG("Changes made - check again");
            log("Changes made - iterate again");
        }
    }
    else
    {
        DBG("NO Changes made - Stuck!");
        log("NO Changes made - Stuck!");
    }
    repaint();
}

bool Sudoku::check_Hidden_Pairs_In_Square(int square)
{
    bool    Result = false;
    int     Tots[N] = { 0 };
    for (int i = 0; i < N ; i++)
            ...
            if (cellToRow(Cell1) == cellToRow(Cell2))
            {
                ...
                for (int n = 0; n < N; n++)
                {
                    ...
                    if (bnCells[cell]->getNote(number + 1))
                    {
                        ...
                        {
                            CauseReported = true;
                            event << "check_Hidden_Pairs_In_Square(" << square + 1 << ") found a pair of " << number + 1 << "s in  Row " << row + 1;
                            DBG(event);
                            log(event);
                        }
                        action.clear();
                        action << "    so cleared " << number + 1 << " in cell " << cellToRowCol(cell);
                        DBG(action);
                        log(action);
            ...
            else if (cellToCol(Cell1) == cellToCol(Cell2))
            {
                ...
                for (int n = 0; n < N; n++)
                {
                    ...
                    if (bnCells[cell]->getNote(number + 1))
                    {
                        ...
                        {
                            CauseReported = true;
                            event << "check_Hidden_Pairs_In_Square(" << square + 1 << ") found a pair of " << number + 1 << "s in  Col " << col + 1;
                            DBG(event);
                            log(event);
                        }
                        action.clear();
...
bool Sudoku::check_Hidden_Matched_Pairs_In_Row(int row)
{
    bool    Result = false;
    int     Tots[N] = { 0 };
    int     p1 = 0;
...
bool Sudoku::check_Hidden_Matched_Pairs_In_Col(int col)
{
    bool    Result = false;
    int     Tots[N] = { 0 };
    int     p1 = 0;
...
bool Sudoku::check_Hidden_Matched_Pairs_In_Square(int square)
{
    bool    Result = false;
    int     Tots[N] = { 0 };
    int     p1 = 0;
    ...
    //DBG("check_Hidden_Matched_Pairs_In_Square " << square + 1);
    for (int i = 0; i < N ; i++)
    {
        int Idx = getIdxxSqr(i, square);
        for (int number = 0; number < N; number++)
                                ...
                                else if (Cell1 >= 0)
                                {
                                    ...
                                    for (int num = 0; num < 9; num++)
                                    {
                                        ...
                                        if (bnCells[Cell1]->getNote(num + 1))
                                        {
                                            ...
                                            {
                                                CauseReported = true;
                                                event << "check_Hidden_Matched_Pairs_In_Square(" << square + 1 << ") found " << pair.toString() << " in " << cellToRowCol(Cell1) << " and " << cellToRowCol(Cell2);
                                                DBG(event);
                                                log(event);
                                            }
                                            String action;
                                            action << "    so (S) cleared " << num + 1 << " in cell " << cellToRowCol(Cell1);
                                            DBG(action);
                                            log(action);
                                        ...
                                        if (bnCells[Cell2]->getNote(num + 1))
                                        {
                                            ...
                                            action << "    so (S) cleared " << num + 1 << " in cell " << cellToRowCol(Cell2);
                                            DBG(action);
                                            log(action);
                                    ...
                                    if (cellToRow(Cell1) == cellToRow(Cell2))  //  Hidden pair in a row
                                    {
                                        ...
                                        for (int r = 0; r < N; r++)
                                        {
                                            ...
                                            if (bnCells[idx]->getNote(p1 + 1))
                                            {
                                                ...
                                                action << "    so (R) cleared " << p1 + 1 << " in cell " << cellToRowCol(idx);
                                                DBG(action);
                                                log(action);
                                                bnCells[idx]->clearNote(p1 + 1);
                                                Result = true;
                                            ...
                                            if (bnCells[idx]->getNote(p2 + 1))
                                            {
                                                ...
                                                action << "    so (R) cleared " << p2 + 1 << " in cell " << cellToRowCol(idx);
                                                DBG(action);
                                                log(action);
                                                bnCells[idx]->clearNote(p2 + 1);
                                                Result = true;
                                    ...
                                    else if (cellToCol(Cell1) == cellToCol(Cell2))  //  Hidden pair in a col
                                    {
                                        ...
                                        for (int c = 0; c < N; c++)
                                        {
                                            ...
                                            if (bnCells[idx]->getNote(p1 + 1))
                                            {
                                                ...
                                                action << "    so (C) cleared " << p1 + 1 << " in cell " << cellToRowCol(idx);
                                                DBG(action);
                                                log(action);
                                                bnCells[idx]->clearNote(p1 + 1);
                                                Result = true;
                                            ...
                                            if (bnCells[idx]->getNote(p2 + 1))
                                            {
                                                ...
                                                action << "    so (C) cleared " << p2 + 1 << " in cell " << cellToRowCol(idx);
                                                DBG(action);
                                                log(action);
                                                bnCells[idx]->clearNote(p2 + 1);
                                                Result = true;
...
bool Sudoku::check_Pairs_In_Row(int row)
{
    int IdxA = 0;
    int IdxB = 0;
    bool    Changes = false;

    //DBG("check_Pairs_In_Row " << row + 1);
    for (int i = 0; i < N-1; i++)
    {
            ...
            if (*matchedPairs[IdxA] == *matchedPairs[IdxB])
            {
                ...
                for (int k = 0; k < N; k++)
                {
                    ...
                    if (bnCells[Idx]->getNote(matchedPairs[IdxA]->get1st()))
                    {
                        ...
                        {
                            CauseReported = true;
                            event << "check_Pairs_In_Row(" << row + 1 << ") found " << matchedPairs[IdxA]->toString() << " in " << cellToRowCol(IdxA) << " and " << cellToRowCol(IdxB);
                            DBG(event);
                            log(event);
                        }
                        action << "    so cleared " << matchedPairs[IdxA]->get1st() << " in " << cellToRowCol(Idx);
                        DBG(action);
                        log(action);
                    ...
                    if (bnCells[Idx]->getNote(matchedPairs[IdxA]->get2nd()))
                    {
                        ...
                        action << "    so cleared " << matchedPairs[IdxA]->get2nd() << " in " << cellToRowCol(Idx);
                        DBG(action);
                        log(action);
...
bool Sudoku::check_Pairs_In_Col(int col)
{
    int IdxA = 0;
    int IdxB = 0;
    bool    Changes = false;

    //DBG("check_Pairs_In_Col " << col + 1);
    for (int i = 0; i < N - 1; i++)
    {
            ...
            if (*matchedPairs[IdxA] == *matchedPairs[IdxB])
            {
                ...
                for (int k = 0; k < N; k++)
                {
                    ...
                    if (bnCells[Idx]->getNote(matchedPairs[IdxA]->get1st()))
                    {
                        ...
                        {
                            CauseReported = true;
                            event << "check_Pairs_In_Col(" << col + 1 << ") found " << matchedPairs[IdxA]->toString() << " in " << cellToRowCol(IdxA) << " and " << cellToRowCol(IdxB);
                            DBG(event);
                            log(event);
                        }
                        action << "    so cleared " << matchedPairs[IdxA]->get1st() << " in " << cellToRowCol(Idx);
...
bool Sudoku::check_Pairs_In_Square(int square)
{
    int IdxA = 0;
    int IdxB = 0;
    bool    Changes = false;
            ...
            if (*matchedPairs[IdxA] == *matchedPairs[IdxB])
            {
                ...
                for (int k = 0; k < N; k++)
                {
                    ...
                    if (bnCells[Idx]->getNote(matchedPairs[IdxA]->get1st()))
                    {
                        ...
                        {
                            CauseReported = true;
                            event << "check_Pairs_In_Square(" << square + 1 << ") found " << matchedPairs[IdxA]->toString() << " in " << cellToRowCol(IdxA) << " and " << cellToRowCol(IdxB);
                            DBG(event);
                            log(event);
                        }
                        action << "    so cleared " << matchedPairs[IdxA]->get1st() << " in " << cellToRowCol(Idx);
...
//    DBG("onlyInRow " << row +1);
    ...
    for (num = 0; num < N; num++)
    {
        ...
        if (Tots[num] == 1)
        {
            ...
            event.clear();
            event << "The only " << num+1 << " in Row " << row+1 << ".    Entered " << num + 1 << " in " << cellToRowCol(Idx);
            DBG(event);
            log(event);
            CurrentCell = Idx;
...
//    DBG("onlyInCol " << row +1);
    ...
    for (num = 0; num < N; num++)
    {
        ...
        if (Tots[num] == 1)
        {
            ...
            event << "The only " << num + 1 << " in Col " << col + 1 << ".    Entered " << num + 1 << " in " << cellToRowCol(Idx);  
            DBG(event);
            log(event);
            CurrentCell = Idx;
            CurrentRow = cellToRow(Idx);
...
bool Sudoku::onlyInSquare(int square)
{
//    DBG("onlyInSquare " << row +1);
    bool    hit = false;
    int     num = -1;
    int     Idx = 0;
    int     Tots[N] = { 0 };
    ...
    for (num = 0; num < N; num++)
    {
        ...
        if (Tots[num] == 1)
        {
            ...
            event << "The only " << num + 1 << " in Square " << square + 1 << ". Entered " << num+1 << " in " << cellToRowCol(Idx);
            DBG(event);
            log(event);
            CurrentCell = Idx;
            CurrentRow = cellToRow(Idx);
...
void Sudoku::resized()
{
    ...
    auto test = label.removeFromLeft(size);
    test.removeFromTop(size / 5);
    test.removeFromBottom(size / 5);
    bnTest->setBounds(test.reduced(4));
...
void Sudoku::createButtons()
{
    ...
    //
    bnTest.reset(new TextButton("Test"));
    addAndMakeVisible(bnTest.get());
    bnTest->setColour(TextButton::buttonOnColourId, Colours::red);
    bnTest->setColour(TextButton::buttonColourId, Colours::orange);
    bnTest->addListener(this);
    //
